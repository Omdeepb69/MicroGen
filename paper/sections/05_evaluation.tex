...
\input{tables/table3_memory_ablation.tex}
...
\input{tables/table2_concurrency_scaling.tex}
...
To verify architectural generalization, Table~\ref{tab:main_results} presents the statistically verified $N=30$ micro-ablation matrix on \texttt{sshleifer/tiny-gpt2} (2.5M parameters), while Figure~\ref{fig:context_scaling} benchmarks architectural context scaling across both \texttt{sshleifer/tiny-gpt2} (2.5M) and \texttt{gpt2} (124M parameters).

\input{tables/table4_generalization_scaling.tex}

Furthermore, Table~\ref{tab:generalization_scaling} evaluates multi-architecture generalization across modern open-weights model families (\texttt{Qwen2.5-0.5B}, \texttt{Llama-3.2-1B}, \texttt{TinyLlama-1.1B}), multi-GPU tensor-parallel scaling on dual NVIDIA Tesla T4 GPUs, and cross-generation hardware duality (NVIDIA Tesla P100 Pascal vs Tesla T4 Turing).

Across modern model families (Category 1), self-attention prefill computation scales quadratically with prompt context length in unoptimized engines, whereas prefix-cached and paged engines maintain sub-linear prefill growth over the prompt suffix. Under \texttt{Qwen2.5-0.5B}, \texttt{Llama-3.2-1B}, and \texttt{TinyLlama-1.1B}, per-tensor INT8 weight quantization achieves $68.1\%$ to $69.7\%$ VRAM reductions ($980\text{ MB} \rightarrow 312\text{ MB}$, $2440\text{ MB} \rightarrow 740\text{ MB}$, and $2200\text{ MB} \rightarrow 670\text{ MB}$) with minor decode throughput variation ($1.02\times$--$1.03\times$ speedups, $p < 0.05$). Combining optimizations (+INT8 +Paged KV +Prefix) induces smaller but directionally consistent non-monotonic composition regressions ($0.97\times$--$0.98\times$ baseline, $p < 0.05$) compared to micro-scale models ($0.96\times$, $p < 0.001^\dagger$), confirming that weight quantization memory savings and multi-optimization composition dynamics generalize across Grouped-Query Attention (GQA) and SwiGLU architectures.

For multi-GPU tensor parallelism (Category 2), sharding linear projections across two T4 GPUs ($TP=2$) accelerates memory-bound decoding for \texttt{gpt2} ($124\text{M}$), increasing decode throughput from $8.2\text{ tok/s}$ to $14.0\text{ tok/s}$ ($1.71\times$ speedup, $p < 0.001^\dagger$). However, for latency-sensitive micro-models (\texttt{tiny-gpt2}), all-reduce synchronization latency dominates over compute savings, resulting in a throughput regression ($0.81\times$ baseline, $p < 0.001^\dagger$).

Finally, cross-generation hardware duality (Category 3) demonstrates the critical role of specialized tensor hardware. On unoptimized FP32 baselines, the Turing Tesla T4 outperforms the Pascal Tesla P100 ($514.1\text{ tok/s}$ vs $357.1\text{ tok/s}$, $1.44\times$ baseline gap) due to Turing's higher Streaming Multiprocessor (SM) execution efficiency, double FP32 issue rate per SM, and higher boost clock frequency ($1590\text{ MHz}$ vs $1480\text{ MHz}$). Under INT8 quantization, hardware lacking INT8 Tensor Cores (NVIDIA P100 Pascal, CC 6.0) suffers a throughput regression ($0.87\times$ baseline, $p < 0.001^\dagger$) due to software scale conversions, whereas Turing-era hardware (NVIDIA T4, CC 7.5 with INT8 Tensor Cores) absorbs quantization conversions efficiently ($1.39\times$ relative throughput vs P100, $p < 0.001^\dagger$).
