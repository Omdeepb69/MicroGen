...
\begin{itemize}
    \item At $0\%$ prefix overlap ($r=0.0$), prefix lookup introduces a small hash lookup overhead ($\delta_{\text{cache}} \approx 0.7\text{ ms}$), resulting in a slight latency penalty ($0.97\times$ speedup, $\text{ns}$).
...
\begin{itemize}
    ...
    \item \textbf{Speculative Decoding Regimes}: Beneficial when target model latency is high and draft model acceptance rate $\alpha > 0.70$. Under small models (\texttt{tiny-gpt2}), draft generation overhead ($4.4\text{ ms}$ per candidate step) exceeds target verification gains, resulting in throughput reduction ($229.0 \pm 6.0\text{ tok/s}$, $0.45\times$ baseline, $p < 0.001^\dagger$).
\end{itemize}
...
Table~\ref{tab:memory_ablation} details memory allocation resilience under VRAM pressure. Standard contiguous KV cache allocation suffers from severe external fragmentation ($35.2\%$ to $81.4\%$) under dynamic sequence length variations, resulting in premature Out-of-Memory (\textsf{OOM}) exceptions at $\le 50\%$ VRAM capacity. In contrast, \textsf{MicroGen}'s \texttt{PagedKVCacheAllocator} dynamically assigns $B_{\text{block}}=16$ token physical blocks, reducing external fragmentation to $0.0\%$ under all capacity regimes and maintaining zero OOM exceptions.
...
\begin{figure}[htbp]
    ...
    \caption{TTFT and TPOT latency scaling across prompt context lengths for \texttt{sshleifer/tiny-gpt2} and \texttt{gpt2} (124M) models.}
    \label{fig:context_scaling}
\end{figure}

\input{tables/table1_main_results.tex}

To verify architectural generalization, Table~\ref{tab:main_results} provides the comprehensive multi-model benchmark matrix across all evaluated configurations. Across both \texttt{sshleifer/tiny-gpt2} (2.5M) and \texttt{gpt2} (124M), context scaling trends remain highly consistent (Figure~\ref{fig:context_scaling}). 

Self-attention prefill computation scales quadratically with prompt context length in unoptimized engines, whereas prefix-cached and paged engines maintain sub-linear prefill growth over the prompt suffix. On host CPU backends (\texttt{hw\_cpu\_*}), INT8 quantization provides an additional throughput speedup due to vectorized SIMD instruction utilization, confirming that \textsf{MicroGen}'s decoupled abstractions generalize across hardware backends.
