...
Across modern model families, self-attention prefill computation scales quadratically with prompt context length in unoptimized engines, whereas prefix-cached and paged engines maintain sub-linear prefill growth over the prompt suffix. Under \texttt{Qwen2.5-0.5B} and \texttt{Llama-3.2-1B}, per-tensor INT8 weight quantization achieves $68.1\%$ to $69.6\%$ VRAM reductions ($980\text{ MB} \rightarrow 312\text{ MB}$ and $2440\text{ MB} \rightarrow 740\text{ MB}$) with negligible decode throughput overhead ($1.03\times$ speedup), confirming that \textsf{MicroGen}'s optimization trade-offs generalize across Grouped-Query Attention (GQA) and SwiGLU architectures.

For multi-GPU tensor parallelism (Category 2), sharding linear projections across two T4 GPUs ($TP=2$) accelerates memory-bound decoding for \texttt{gpt2} ($124\text{M}$), increasing decode throughput from $8.2\text{ tok/s}$ to $14.0\text{ tok/s}$ ($1.71\times$ speedup). However, for latency-sensitive small models (\texttt{tiny-gpt2}), all-reduce synchronization latency dominates over compute savings, resulting in a throughput regression ($0.81\times$ baseline). Finally, cross-generation hardware duality (Category 3) demonstrates that hardware lacking INT8 Tensor Cores (NVIDIA P100 Pascal, CC 6.0) suffers a throughput regression under INT8 quantization ($0.87\times$ baseline), whereas Turing-era hardware (NVIDIA T4, CC 7.5 with Tensor Cores) absorbs quantization conversions efficiently ($1.39\times$ relative speedup vs P100).
